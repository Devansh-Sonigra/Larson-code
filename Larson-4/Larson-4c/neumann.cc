...
template<>
double
ExactSolution<3>::value(const Point<3>& p,
                        const unsigned int /*component*/) const
{
...
template<>
Tensor<1, 3>
ExactSolution<3>::gradient(const Point<3>&   p,
                           const unsigned int) const
{
  Tensor<1, 3> values;
  values[0] = 2 * p[0];
...
template<>
double
RHS_function<3>::value(const Point<3>& p,
                       const unsigned int /*component*/) const
{
  return -6;
}
...
template<>
double
G_Neumann<3>::value(const Point<3>& p, const unsigned int) const
{
  if(p[0] == 0)
  {
    return 0.0;
...
NeumannSolver<dim>::assemble_system()
{
  ...
  system_rhs(dof_handler.n_dofs()) = 0;
}
