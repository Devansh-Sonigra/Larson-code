...
void Projection::make_grid_and_dofs() {
...
void Projection::compute_error(double &L2_error, double &H1_error) {
    ...
    VectorTools::integrate_difference (mapping, dof_handler, solution, exact_solution, difference_per_cell, QGaussSimplex<2>(2*fe.degree+1),VectorTools::H1_seminorm);

    H1_error = difference_per_cell.l2_norm();
}

void Projection::refine_grid() {
    triangulation.refine_global(1);  
}

void Projection::run(std::vector<int> &ncell,
                     std::vector<int> &ndofs,
                     std::vector<double> &L2_error,
                     std::vector<double> &H1_error) {
